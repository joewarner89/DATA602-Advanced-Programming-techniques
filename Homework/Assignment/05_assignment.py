...
def exercise08():
    ...
    # ------ Place code below here \/ \/ \/ ------
    df = pd.read_csv(
        "https://raw.githubusercontent.com/data602sps/assignments/master/Sacramentorealestatetransactions.csv")
    row_count = len(df)
    avg_sq_ft = np.average(df['sq__ft'])
    df_zip_95670 = df[df.zip == 95670]
    df_zip_not_95610 = df[df.zip != 95670]
...
def exercise11(n):
    ...
    # Reshape the array into an array with n/3 columns and 3 rows
...
def exercise12(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    # Create a 2n x 2n matrix of zeros.
    checkerboard_matrix = np.zeros((2 * n, 2 * n))

    # Fill the matrix with a checkerboard pattern.
    for i in range(0, 2 * n, 2):
        for j in range(0, 2 * n, 2):
            checkerboard_matrix[i, j] = 1
            checkerboard_matrix[i + 1, j + 1] = 1
...
def exercise13(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    random_int = np.random.randint(0, n, size=n)
    date_range = pd.date_range(start='1/1/2010', periods=n)
    s = pd.Series(random_int, index=date_range)
    # Plotting the cummulitative sum
...
def exercise14(words):
    ...
    word = map(lambda word: len(word), words)

    # Convert the mapped lengths into a pandas Series
    df = pd.Series(word)
